...
#define TESTLENGTH PDM_BUFFERSIZE
...
static q31_t tmp_rms_values[BP_CHANNELS];
static float32_t rms_values_l[BP_CHANNELS];
static float32_t rms_values_r[BP_CHANNELS];
...
int32_t audio_dsp_band_pass_filter_bank(q31_t *bufferSrc, size_t size, bool l_or_r) {
    ...
    for (i = 0; i < (int32_t)size / BLOCKSIZE; i++) {

        /* ----------------------------------------------------------------------
        ** Convert block of input data from float to Q31
        ** ------------------------------------------------------------------- */

        memcpy((int32_t *)inputQ31, (int32_t *)(bufferSrc[0] + (i * BLOCKSIZE)), BLOCKSIZE * sizeof(q31_t));
        /* ----------------------------------------------------------------------
        ...
        ** ------------------------------------------------------------------- */
        ...
        arm_rms_q31(outputQ31, BLOCKSIZE, &tmp);
        tmp_rms_values[0] += tmp;
        arm_biquad_cas_df1_32x64_q31(&S2, outputQ31, outputQ31, BLOCKSIZE);
        arm_rms_q31(outputQ31, BLOCKSIZE, &tmp);
        tmp_rms_values[1] += tmp;
        /* ----------------------------------------------------------------------
    ...
    if (i > 0) {
        /* Do mean of RMS Values */
        for (int j = 0; j < BP_CHANNELS; j++) {
            q31_t t1 = tmp_rms_values[i];
            tmp_rms_values[i] = t1 / i;
        }
        (l_or_r) ? (arm_q31_to_float(tmp_rms_values, rms_values_l, BP_CHANNELS)) : (arm_q31_to_float(tmp_rms_values, rms_values_r, BP_CHANNELS));
    }
    return i * BLOCKSIZE;
}
