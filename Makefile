...
$(OUTPUT_DIR)/$(PROJECTNAME).out: $(OBJS) $(LIB_FILES) | $(OBJ_DIRS)
	@echo 'Linking $(OUTPUT_DIR)/$(PROJECTNAME).elf'
	$(ECHO)$(LD) $(LD_FLAGS) $(OBJS) $(LIBS) -o $(OUTPUT_DIR)/$(PROJECTNAME).elf
	$(ECHO)$(OBJCOPY) $(OUTPUT_DIR)/$(PROJECTNAME).elf -O binary $(OUTPUT_DIR)/$(PROJECTNAME).bin
	$(ECHO)$(OBJCOPY) $(OUTPUT_DIR)/$(PROJECTNAME).elf -O ihex $(OUTPUT_DIR)/$(PROJECTNAME).hex
	$(ECHO)$(OBJCOPY) $(OUTPUT_DIR)/$(PROJECTNAME).elf -O srec $(OUTPUT_DIR)/$(PROJECTNAME).s37
	@echo 'Done.'
